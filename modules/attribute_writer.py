...
def extract_allowed_columns(header_row):
    category_to_columns = {}
    for col_idx, raw_header in enumerate(header_row):
        ...
        # Parse category from prefix (e.g., "TOP: Sleeve Length (1)")
        if ":" in raw_header:
            category, attr = [s.strip() for s in raw_header.split(":", 1)]
        else:
            category, attr = "_GLOBAL", raw_header  # shared across all
...
def select_values_for_category(df, col_indices):
    ...
    for col_idx, attr_name in col_indices:
        try:
            ...
            output_row[attr_name] = ", ".join([str(v).strip() for v in selected if pd.notna(v)])
...
def write_marketplace_attribute_sheet(df, pdf_filename, creds, folder_id):
    ...
    for tab_name in ["faire", "fgo"]:
        ...
        header_row = tab_df.columns.tolist()
        value_df = tab_df.dropna(how="all")

        category_to_columns = extract_allowed_columns(header_row)

        output_rows = []
        for _, row in df.iterrows():
            style_number = row.get("Style Number", "")
            product_type = (row.get("Product Type") or row.get("Product Category") or "").strip()

            col_indices = category_to_columns.get("_GLOBAL", []) + category_to_columns.get(product_type, [])
        ...
        all_headers = ["Style Number"] + sorted({attr for pair_list in category_to_columns.values() for _, attr in pair_list})
